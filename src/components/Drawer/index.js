        ...
        <div className={`${styles.overlay} ${opened ? '' : styles.overlayVisible }`}>
            <div className={styles.drawer}>
                <h2 className='text-2xl flex justify-between'>
                    Корзина
                    <img onClick={() => closeCart()} src="react_sneaker/img/remove_snake.svg" className='removeBtn' alt="Remove"/>
                </h2>

                {
                    items.length > 0 ?
                            ...
                            <div className="items">
                                {items.map((item, index) => (
                                    <div key={index} className='cartItem border rounded-lg flex p-4 text items-center'>
                                        ...
                                        <img onClick={() => onRemove(item.id)} src="react_sneaker/img/remove_snake.svg" className='removeBtn' alt="Remove"/>
                                    </div>
                                ))}
                            </div>
                        ...
                        :
                        <Info title={isComplete ? `Заказ № - ${orderId} оформлен` : "Корзина пуста" } description={isComplete ? "Вернуться в магазин" : "Добавьте товар в корзину"  } image={isComplete ? "react_sneaker/img/issued.png" : "react_sneaker/img/cart-null.png" }/>
                }
